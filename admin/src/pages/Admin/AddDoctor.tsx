...
export default function AddDoctor() {
  ...
  if (!adminContext) {
    throw new Error("AdminContext is null. Ensure the provider is wrapping the component.");
  }

  const { backendUrl, adminToken } = adminContext;
  ...
  const onSubmitHandler = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (docImg) {
        formData.append('image', docImg);  // Ensure docImg is a File object
      }
      formData.append('name', name);
      formData.append('email', email);
      formData.append('password', password);
      formData.append('experience', experience);
      formData.append('fees', fees);  // Convert to number
