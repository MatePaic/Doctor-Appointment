...
export default function Doctors() {
  ...
  const applyFilter = () => {
    if (speciality) {
      setFilterDoc(doctors.filter((doctor: Doctor) => doctor.speciality === speciality));
    } else {
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4  gap-4 gap-y-6">
          {
            filterDoc.map((doctor: Doctor, index: number) => (
              <div onClick={() => navigate(`/appointment/${doctor._id}`)} key={index} 
                className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500'>
                  <img src={doctor.image} alt="image" className='bg-blue-50' />
                  <div className='p-4'>
